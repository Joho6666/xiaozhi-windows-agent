...
通过标准的 **Model Context Protocol (MCP)**，小智可以调用 Windows 本地工具、执行任务、搜索/读写文件、控制浏览器、调用本地 Codex AI 分析代码以及截取屏幕进行视觉分析。
...
小智 ESP32 硬件 (麦克风/喇叭/屏幕)
 ...
 XiaoZhi Windows Agent Bridge (本机 Python 进程)
       ↓
  Tool Registry (注册 46 个本地能力工具)
       ↓
┌─────────────────┬─────────────────┬─────────────────┬─────────────────┐
│ Windows 应用    │ 本地文件系统    │ 终端与 PowerShell│ Codex 编程引擎  │
│ • 智能启动/搜索 │ • 全盘/目录遍历 │ • Safe Terminal │ • 只读分析      │
│ • 进程聚焦/退出 │ • 文件读写/删除 │ • 任意 Shell    │ • 代码修改      │
├─────────────────┼─────────────────┼─────────────────┼─────────────────┤
...
| **代码分析** | *“小智，让 Codex 检查项目为什么报错”* | `codex_task(mode="analyze")` |
...
Connected; registered 46 local tools
```
